...
👉 Use the *💸 Withdraw* button below when you’re ready to cash out!""",
        ...
        if step == "name":
            ...
            user_states[user_id] = {"step": "code", "name": name}
                ...
                if req > bal:
                    ...
                    for idx, row in enumerate(rows[1:], start=2):
                        if row[2] == user_id:
                            ...
                            bot.send_message(message.chat.id,
                                f"✅ ₹{req} withdrawal requested. Remaining balance: ₹{updated_bal}",
                                reply_markup=main_menu())
                            break
                    user_states[user_id] = {"step": None}
            except ValueError:
    ...
    while True:
        ...
        try:
            ...
            for i in range(2, len(rows)+1):
                sheet.update_cell(i, 11, "0")  # Reset 'Daily_Sales' column to 0
            print("✅ Daily_Sales column reset successfully.")
        except Exception as e:
            print(f"⚠️ Error resetting Daily_Sales: {e}")
